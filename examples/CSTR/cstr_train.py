...
audio_transforms = Compose([QuantiseULaw(u = 255), Normalize(2, 1), Lambda(lambda x: x.astype(np.long)), FixLength1D(15 * 16000), Numpy2Tensor()])
...
train_loader = AsynchronousLoader(data_set, device = torch.device('cuda:0'), batch_size = 2, shuffle = False, sampler = train_sampler)
validation_loader = AsynchronousLoader(data_set, device = torch.device('cuda:0'), batch_size = 2, shuffle = False, sampler = validation_sampler)

model = CSTRCharacterModel().cuda()

loss = [nn.NLLLoss(ignore_index = 0).cuda()]
