...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        probs = Categorical(F.softmax(self.policy_out(features), dim=1))
        return probs, self.value_out(features)
...
env = [gym.make('LunarLander-v2') for i in range(args.envs)] # Do it concurrently
...
for j in range(args.episode_len):
    with torch.no_grad():
        ...
        action = torch.argmax(out_distribution.probs).item()
