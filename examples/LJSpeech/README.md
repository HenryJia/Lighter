...
``` python wavenet_run.py --csv_dir [LJSpeech metadata.csv location] --audio_dir [LJSpeech wav file directory] <additional arguments>```  
  
You can also specify arguments via json using ``` --json config.json``` and I provide a json file of the configuration I used. Note you should train and generate using the same arguments/json configuration.  
  
Note since WaveNet is **NOT** a full text to speech model, we need the Mel spectrograms so we still need to dataset to try out generation.  
