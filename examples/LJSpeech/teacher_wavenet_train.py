...
from lighter.train.metrics import GaussianNLLLoss
...
sample = data_set[0][1] # Compute a mu and sigma for our priors, we only need 1 sample for this
mu, log_sigma = torch.mean(sample), torch.log(torch.std(sample))
...
model = MelModel(n_mels = args['n_mels'], n_fft = args['n_fft'], hops = args['hops'],
                 ...
                 out_channels = 2, embed = False, bias = True, mu = mu, log_sigma = log_sigma)
...
validation_callback = train_callbacks + [CheckpointCallback(args['model_name'], monitor = 'GaussianNLLLoss_0', save_best = True, mode = 'max')]
